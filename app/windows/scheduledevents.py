...
class ScheduledEventsWindow(BaseGUIWindow):
    ...
    def window(cls) -> sg.Window:
        ...
        weekday_events = AttendanceSession.objects.filter(event_type__in=(EventTypeChoices.LECTURE, EventTypeChoices.LAB), start_time__week_day=weekday_mapping[today.weekday()][0])
        scheduled_events = AttendanceSession.objects.filter(start_time__day=today.day, start_time__month=today.month, start_time__year=today.year, initiator__isnull=True)
        
        weekday_event_dict = list(weekday_events.values("id","course__code", "event_type", "start_time", "duration"))
        scheduled_event_dict = list(scheduled_events.values("id", "course__code", "event_type", "start_time", "duration"))
        
        # save list index-to-AttendanceSession ID mapping to sections
        # of the configparser
        if scheduled_event_dict:
            app_config.cp["scheduled_events"] = {idx:k["id"] for idx, k in enumerate(scheduled_event_dict)}
        if weekday_event_dict:
            app_config.cp["weekly_events"] = {idx:k["id"] for idx, k in enumerate(weekday_event_dict)}
        ...
        for idx, record in enumerate(scheduled_event_dict, 1):
            scheduled_event_rows.append(
                [
                    idx,
                    ''.join(['  ', record["course__code"], ' ', EventTypeChoices(record["event_type"]).label[:4], '.  ']),
        ...
        for idx, record in enumerate(weekday_event_dict, 1):
            ...
            weekly_event_rows.append(
                [
                    idx, 
                    ''.join([record["course__code"], ' ', EventTypeChoices(record["event_type"]).label[:4], '.']), 
                    ''.join([event_weekday[:4], ' ', record["start_time"].strftime("%H:%M")]),
                    record["duration"]
                ]
            )
        ...
        layout = [
            ...
            [
                sg.Push(),
                sg.Table(scheduled_event_rows, headings=table_headings, k="scheduled_events", enable_click_events=True, justification='center'),
                sg.Push()
            ],
            [sg.HorizontalSeparator()],
            [sg.Push(), sg.Text("Weekly Events"), sg.Push()],
            [sg.Table(weekly_event_rows, headings=table_headings, k="weekly_events", enable_click_events=True, justification='center')],
        ...
        window = sg.Window("Scheduled Events", scrolled_layout, **cls.window_init_dict())
        return window
    ...
    def loop(cls, window: sg.Window, event: str, values: Dict[str, Any]) -> bool:
        ...
        if "scheduled_events" in event and event[2][0] is not None:
            ...
            confirm_start = sg.popup_yes_no("Starting scheduled event %s. Continue?" % event_obj.course.code, title="Start Event", keep_on_top=True)
        ...
        if "weekly_events" in event and event[2][0] is not None:
            ...
            confirm_start = sg.popup_yes_no("Starting weekly event %s. Continue?" % existing_obj.course.code, title="Start Event", keep_on_top=True)
            if confirm_start != "Yes": 
                return True
            ...
            new_event_dict = {
                ...
                "start_time": datetime(year=today.year, month=today.month, day=today.day, hour=existing_obj.start_time.hour, minute=existing_obj.start_time.minute, tzinfo=timezone.utc),
                "duration": existing_obj.duration,
                "node_device": existing_obj.node_device,
                "session": existing_obj.session,
            }
            try:
                new_event_obj, _ = AttendanceSession.objects.get_or_create(**new_event_dict)
