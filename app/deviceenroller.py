...
class DeviceEnroller:
    ...
    """
    DEVICE_CONFIG_FILE_PATH = os.path.join(Path(os.path.abspath(__file__)).parent, '.device_config.ini')
    ...
    def device_id(self):
        ...
        if not self.device_config.has_option("device_registration", "device_id"):
    ...
    def device_name(self):
        if not self.is_device_registered():
            raise Exception("Device not registered")
        if not self.device_config.has_option("device_registration", "device_name"):
            raise Exception("Device improperly configured")
    ...
    def device_token(self):
        if not self.is_device_registered():
            raise Exception("Device not registered")
        if not self.device_config.has_option("device_registration", "device_token"):
            raise Exception("Device improperly configured")
